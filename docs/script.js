...
// ---------------- TEAM LAYOUT (YOUR RULES) ----------------
// Desktop grid is 4 columns.
// We assign spans PER ROW (groups of 4 items):
// row has 1 -> [4]
// row has 2 -> [2,2]
// row has 3 -> [1,1,2]
// row has 4 -> [1,1,1,1]
(function initTeamLayout() {
  const grid = document.getElementById("teamGrid");
  if (!grid) return;

  const cards = () => Array.from(grid.querySelectorAll(".team-card"));

  function clearSpans(list) {
    list.forEach((el) => (el.style.gridColumn = ""));
  }

  function applySpans() {
    const list = cards();

    // If we're on mobile (team grid becomes 1 column), don't force spans
    ...
    for (let start = 0; start < list.length; start += 4) {
      const row = list.slice(start, start + 4);
      const r = row.length;

      if (r === 1) {
        row[0].style.gridColumn = "span 4";
      } else if (r === 2) {
        row[0].style.gridColumn = "span 2";
        row[1].style.gridColumn = "span 2";
      } else if (r === 3) {
        row[0].style.gridColumn = "span 1";
        row[1].style.gridColumn = "span 1";
        row[2].style.gridColumn = "span 2";
      } else {
        // r === 4
        row.forEach((el) => (el.style.gridColumn = "span 1"));
      }
    }
  }
